...
const CommentsForm = ({ slug, postId }) => {
  ...
  useEffect(() => {
    if (nameEl.current) nameEl.current.value = window.localStorage.getItem('name') || '';
    if (emailEl.current) emailEl.current.value = window.localStorage.getItem('email') || '';
  }, [])
  ...
  const handleCommentSubmission = () => {
    ...
    const commentObj = { name, email, comment, postId };
